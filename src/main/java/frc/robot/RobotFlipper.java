...
public class RobotFlipper {
    ...
    void runClimber() {
        ...
        // double stickInput =
        // Robot.climbController.getRawAxis(Constants.CLIMBER_CONTROLLER_AXIS);
        ...
        // // set the motor (which sets the other automatically) to run based on
        // joystick
        ...
        if (Robot.operatorController.getBumper(Hand.kLeft)) {
            ...
            if (Math.abs(stickInput) > 0.2) {
                climbDrive.tankDrive(dirConstant * climbPower * Math.pow(Math.abs(stickInput), climbExponent),
                        dirConstant * climbPower * Math.pow(Math.abs(stickInput), climbExponent));
            } else {
                climbDrive.tankDrive(0, 0);
            }
        } else {
